...
namespace VP.Nest.System.Scripts.Editor.PlayerPrefsEditor
{
	public class PlayerPrefsEditor : EditorWindow
	{
		...
		private int addedPlayerPrefs = 0;
		...
		private readonly GUIContent btnAddPlayerPref = new GUIContent("Add a PlayerPref", "Adds a new key-value pair for you to fill and save");
		...
		private void OnGUI()
		{
			...
			if (GUILayout.Button(btnAddPlayerPref, EditorStyles.miniButton, GUILayout.MaxWidth(120)))
			{
				addedPlayerPrefs++;
				string[] tempKeys = addedKeys;
				string[] tempValues = addedValues;
		...
		{
			if (values.Length > 0)
				EditorGUILayout.LabelField("Current PlayerPrefs", EditorStyles.label);
			else
				EditorGUILayout.LabelField("There is no PlayerPref found!", EditorStyles.label);
			...
			for (int i = 0; i < values.Length; i++)
			{
				...
				else if (keyValues[i].Type.Equals(PlayerPrefsExtension.PlayerPrefPair.PrefType.Float))
				{
					...
					values[i] = EditorGUILayout.FloatField(value).ToString(CultureInfo.InvariantCulture);
				}
				else if (keyValues[i].Type.Equals(PlayerPrefsExtension.PlayerPrefPair.PrefType.Int))
				{
					int value = -1;
					int.TryParse(values[i], out value);
					values[i] = EditorGUILayout.IntField(value).ToString();
				}
		...
		private IEnumerator ProgressBarCoroutine(string startProgressTitle, string doneProgressTitle, string doneMessage)
		{
			...
			EditorWaitForSeconds waitForOneSecond = new EditorWaitForSeconds(0.1f);
		...
		{
			int removedPrefs = 0;
			for (int i = 0; i < keyValues.Count + removedPrefs; i++)
			{
				if (keyValues[i - removedPrefs].Key.ToLower().Contains("unity"))
				{
					keyValues.RemoveAt(i - removedPrefs);
					removedPrefs++;
				}
			}
		}
		...
		private void Message(string _message, MessageType _messageType)
		{
			message = _message;
